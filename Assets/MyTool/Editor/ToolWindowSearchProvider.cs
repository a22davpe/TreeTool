...
namespace MyTool
{
    public class ToolWindowSearchProvider : ScriptableObject, ISearchWindowProvider
    {
        ...
        {
            var windowMousePosition = tool.ChangeCoordinatesTo(tool, context.screenMousePosition - tool.window.position.position);
            var toolMousePosition = tool.contentViewContainer.WorldToLocal(windowMousePosition);

            SearchContextElement element = (SearchContextElement)SearchTreeEntry.userData;

            ToolNode node = (ToolNode)element.target;
            node.SetPosition(new Rect(toolMousePosition, new Vector2()));
            tool.Add(node);
            return true;
